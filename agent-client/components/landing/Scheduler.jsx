    ...
    <section className="py-32 container mx-auto px-6">
      <div className="text-center mb-16">
        <div className="inline-flex relative items-center gap-2 px-3 py-1 rounded-full bg-green-500/10 border border-green-500/20 text-sm text-green-400 mb-6">
          ...
          <span className="absolute -top-2 -right-12 rotate-30 bg-amber-600 text-white px-2 rounded-full">Coming soon</span>
        </div>
        <h2 className="text-4xl font-bold text-white mb-4">
          Plan Once, Post Everywhere
        </h2>
        <p className="text-gray-400">
          Drag, drop, and let AI handle the best posting times.
        </p>
      </div>
            ...
            <div key={time} className="grid grid-cols-8 gap-4 h-24">
              ...
              {days.map((_, dayIndex) => {
                const post = posts.find(
                  (p) => p.day === dayIndex && p.time === timeIndex
                );

                return (
                  <div
                    key={dayIndex}
                    className="relative border border-white/5 rounded-xl bg-white/5 hover:bg-white/10 transition-colors group"
                  >
